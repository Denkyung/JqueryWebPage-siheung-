...
function init() {
  ...
  try { 
    ...
    var db_password = 'tempPassword';  //6번
...
function chartData_real(req, res, excelFlag, callbackFunc) {
  try {
    ...
    //[A] 비동기 쿼리를 sum하기 위해 promise로 실행 
...
function makeChartOption(fromDate, toDate, strChartTimeUnit, arrQueryRet) {  
    ...
    for(var i=0; i<arrQueryRet.length; i++) {
      ...
      var cartDataJson = {
        ...
        stack: 'Total',
        type: 'line',
        etcXDisplayUnit : arrXUnitString, //X축 단위 - 차트 스펙에는 없지만 추가적으로 더 보냄
        etcXTimeline : arrTimeLine, //x축 실제 시간 - 차트 스펙에는 없지만 추가적으로 더 보냄 
        etcTotalCount: totalCount //총 개수 - 차트 스펙에는 없지만 추가적으로 더 보냄 
      }
